...
def _run(data_df, template_path, mapping, recipient_col,
         email_subj, email_body_plain, email_body_html,
         filename_pattern, auth_user, auth_pwd, cert_dir, failed_path):
    """Execute the full generate-and-send pipeline."""
    total = len(data_df)
    sent = 0
    failed = []
    ...
    for i, (row, context, pdf_path) in enumerate(pdf_results):
        ...
        email_pct = int(((i + 1) / max(len(pdf_results), 1)) * 50)
        task_state.progress = 50 + email_pct
    ...
    task_state.log(f"═══ Complete: {sent}/{total} sent, {len(failed)} failed ═══")
